...
namespace TicTacToe.Forms.Game.NetworkGame
{
	...
	{
		private const int LOBBY_PREVIEW_PANEL_HEIGHT = 100;

		private (Color Default, Color Selected) _lobbyPreviewPanelColor =
			(Color.FromArgb(45, 45, 45), Color.FromArgb(200, 229, 158, 31));
		...
		private void CreateExampleData(System.Net.IPAddress ip)
		{
			var lpea = new LobbyPreviewEventArgs(ip, 7755,
				new Models.GameInfo.Settings.NetworkGameSettings(new string('W', 55), 1000, "WWWWWWWWWWWWWWWWWWWW", // Example Data
				new Models.PlayerItem.Avatar(), 99, Models.GameInfo.Settings.FieldSize.Size5on5, false, false, 1, 2));
		...
		private void CreateLobbyPreview(object sender, LobbyPreviewEventArgs e)
		{
			...
			Label labelOpponentName = CreateLabelOpponentName($"#{_lobbyPreviewNumber++}  {e.NetworkGameSettings.OpponentName}");
			...
			Label labelCoinBet = CreateLabelCoinBet(e.NetworkGameSettings.CoinBet,
			...
			panel.Controls.Add(labelCoinBet);
		...
		{
			const int LABEL_MARGIN_LEFT = 25;
			...
			Label label = new Label
			{
				...
				Location = new Point(LABEL_MARGIN_LEFT, LABEL_MARGIN_TOP)
			};
		...
		private Label CreateLabelDescription(string description)
		{
			...
			label.Location = new Point(LABEL_MARGIN_LEFT, (LOBBY_PREVIEW_PANEL_HEIGHT + label.Height) / 3);
			label.Click += LobbyPreview_Click;
			label.DoubleClick += LobbyPreview_DoubleClick;
		...
		private Label CreateLabelCoinBet(int coinBet, int labelMarginLeft)
		{
			string coinBetText = coinBet == 0 ?
				"Free" :
				$"{coinBet:N0}".Replace(',', ' ');
			Label label = new Label
			{
